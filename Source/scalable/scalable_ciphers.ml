...
*)
let  monki_flip bitarray=
  let rec funny_monki = function
    |[]->[]
    |e::l->(Random.int (1))::funny_monki l
  in quot_b (funny_monki bitarray) [0;0;1];;
   
  
let rec public_data_g p = let g = monki_flip p in (g,p);;

(** Generate ElGamal public data.
    @param pub_data a tuple (g, p) of public data for ElGamal cryptosystem.
 *)
let generate_keys_g (g, p) =
  let a = add_b(monki_flip (diff_b p [0;1])) [0;1]
...
let encrypt_g msg (g, p) kA =
  let k = add_b(monki_flip (diff_b p [0;1])) [0;1] in
