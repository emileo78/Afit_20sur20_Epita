 ...
 *)
let rec public_data_g p = let g = Random.int p in (g,p);;

(** Generate ElGamal public data.
    @param pub_data a tuple (g, p) of public data for ElGamal cryptosystem.
 *)
let generate_keys_g (g, p) = let a = Random.int (p-1) + 1 in (mod_power g a p, a);;
...
let encrypt_g msg (g, p) kA =
	  let k = Random.int (p-1) +1 in (mod_power g k p, modulo (msg*(mod_power kA k p)) p);;
...
let decrypt_g (msgA, msgB) a (g, p) =
	  let k = mod_power msgA a p in modulo (msgB * mod_power k (p-2) p ) p;;
